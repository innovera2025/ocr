...
The web entrypoint runs the idempotent, checksum-verified migrations in `prisma/migrations`; the worker verifies database readiness and that the newest migration it needs (`0018_multipage_documents`; Deploy B of Release 2 raises that gate to `0020_batch_round_clock`) is recorded in `schema_migrations` before accepting work; otherwise it exits and Docker restarts it. The compose file does not run destructive migrations outside that runner; the migration role must have permission to apply them.
