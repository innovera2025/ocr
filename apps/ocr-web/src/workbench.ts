...
const total=Math.max(1,done?rows:rows+Math.max(0,pagesExpected-pages)+Math.max(0,expected-uploaded));
...
const missing=expected-uploaded;$('batch-meta').textContent=[b.label||'','เริ่ม '+when(b.createdAt),'ไฟล์ '+uploaded+'/'+expected,pagesExpected>0?(splitting>0?'กำลังแยกหน้า ':'หน้า PDF ')+pages+'/'+pagesExpected:'','อ่านเสร็จ '+completed+' จาก '+total+' แถว',missing>0&&!pending?'อัปโหลดไม่ครบ '+missing+' ไฟล์':''].filter(Boolean).join(' · ');
...
const stats=[['ทั้งหมด (แถว)',total,''],['รอคิว',n('queued'),'queued'],['กำลังอ่าน',n('processing'),'processing'],['อ่านสำเร็จ',n('succeeded'),'succeeded'],['รอตรวจสอบ',n('needsReview'),'review'],['ไม่สำเร็จ',n('failed'),'failed'],['ยืนยันแล้ว',n('confirmed'),'confirmed'],['เวลาที่ใช้',clock(elapsed),''],['เอกสาร/นาที',rate===null||!isFinite(rate)?'—':rate.toFixed(1),'']];
