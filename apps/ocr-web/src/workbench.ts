...
/* Files rejected before upload (or abandoned) never reach the server: once nothing is left to upload, retry or read, the batch is settled. */const retryable=state.uploads.some(u=>u.group&&u.group.batchId===b.batchId&&u.status==='failed'&&u.retryable),settled=!done&&!pending&&!retryable&&inFlight===0&&n('uploaded')>0&&completed>=n('uploaded');
...
$('batch-title').textContent=done?'ชุดอัปโหลดล่าสุด · อ่านครบแล้ว':settled?'ชุดอัปโหลดล่าสุด · อ่านครบทุกไฟล์ที่อัปโหลดได้':'ชุดอัปโหลดล่าสุด · กำลังดำเนินการ';
...
renderUploads();if(!ok.length){notify('ไม่มีไฟล์ที่อัปโหลดได้ (ใช้ได้เฉพาะ PNG, JPG, WebP และ PDF ที่ไม่ว่างและไม่เกิน 200 MB) ดูเหตุผลในรายการอัปโหลด','error');return;}notify('เพิ่ม '+ok.length+' ไฟล์เข้าคิวอัปโหลดแล้ว'+(bad?' · ข้ามไฟล์ที่ใช้ไม่ได้ '+bad+' ไฟล์':''));pump();schedule();}
...
async function loadPreview(seq,id){const box=$('p-content');if(state.current&&String(state.current.status||'').toUpperCase()==='QUARANTINED'){box.replaceChildren(el('p','sub err p-note',ERRORS.DOCUMENT_QUARANTINED));return;}box.replaceChildren(el('p','sub p-note','กำลังโหลดต้นฉบับ…'));
