 ...
 * grey form image per page, like the real 95-page upload), 1 / 12 / 301 pages, /Rotate 90, truncated, encrypted (RC4,
 * with and without a user password) and a huge MediaBox. No customer data.
 */
...
type PageSpec = { widthPt?: number; heightPt?: number; rotate?: number };
...
/** 1400×990 8-bit grey "form": white paper, printed rules and boxes, light noise (compresses like a clean scan). */
function formImage(): Buffer {
  const width = 1400, height = 990, pixels = Buffer.alloc(width * height, 250);
  let seed = 7;
...
function syntheticPdf(pages: readonly PageSpec[], encrypt?: { user: string; owner: string }): Buffer {
  ...
  const imageId = add(`<< /Type /XObject /Subtype /Image /Width 1400 /Height 990 /ColorSpace /DeviceGray /BitsPerComponent 8 /Filter /FlateDecode /Length ${image.length} >>`, image);
  const kids: number[] = [];
  pages.forEach((page, index) => {
    const w = page.widthPt ?? 832, h = page.heightPt ?? 590;
    const content = Buffer.from(`q ${w} 0 0 ${h} 0 0 cm /Im1 Do Q BT /F1 18 Tf 36 36 Td (Synthetic page ${index + 1}) Tj ET`, "latin1");
    const contentId = add(`<< /Length ${content.length} >>`, content);
    kids.push(add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${w} ${h}]${page.rotate ? ` /Rotate ${page.rotate}` : ""} /Resources << /Font << /F1 ${font} 0 R >> /XObject << /Im1 ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`));
  });
...
describe("poppler renderer on synthetic PDFs", { skip: hasPoppler ? false : "poppler-utils (pdfinfo/pdftoppm) not installed: run inside the worker image" }, () => {
  ...
  test("1 page: pdfinfo reads it and pdftoppm renders a 1610 px long side PNG", async (t) => {
    ...
    assert.deepEqual(size, { width: PAGE_LONG_SIDE_PX, height: 1142 }, "832×590 pt → 1610×1142 px, the Local AI's own PDF render size");
    assert.equal(pages.get(1)?.mimeType, "image/png");
  });

  test("12 pages: every page stored under its deterministic key, in page order, at 1610 px", async (t) => {
    ...
    for (let page = 1; page <= 12; page += 1) {
      ...
      assert.equal(Math.max(...Object.values(pngSize(stored))), PAGE_LONG_SIDE_PX);
      assert.equal(pages.get(page)?.sizeBytes, stored.byteLength);
    }
  });

  test("/Rotate 90 is honoured: the landscape page renders portrait with the same 1610 px long side", async () => {
    const { objects } = await split(syntheticPdf([{}, { rotate: 90 }]));
    assert.deepEqual(pngSize(objects.get(childStorageKey(tenant, parentId, 1))!), { width: 1610, height: 1142 });
    assert.deepEqual(pngSize(objects.get(childStorageKey(tenant, parentId, 2))!), { width: 1142, height: 1610 });
  });
  ...
  test("encrypted: a user password is PDF_ENCRYPTED (not retryable); a permissions-only PDF (empty user password) still renders", async () => {
    ...
    assert.equal(Math.max(...Object.values(pngSize(objects.get(childStorageKey(tenant, parentId, 2))!))), PAGE_LONG_SIDE_PX);
  });
